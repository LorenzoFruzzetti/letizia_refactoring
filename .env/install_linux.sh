...
# ---------------------------------------------------------------------------
# System-level note: the ROI editor (roi_editor.py) is a pyqtgraph (Qt) window.
# The Python packages come from the env spec, but a Qt binding on a headless
# Linux box often still needs system X/GL libraries, so this is checked and
# reported rather than left to fail at first launch.
# ---------------------------------------------------------------------------
report_gui() {
    local py="$1"
    if "${py}" -c "import pyqtgraph" >/dev/null 2>&1; then
        # Importing pyqtgraph is not proof the window will open: the Qt platform
        # plugin is what needs the system libraries. Try to build a QApplication
        # offscreen, which exercises the binding without needing a display.
        if QT_QPA_PLATFORM=offscreen "${py}" -c \
            "import pyqtgraph as pg; pg.mkQApp()" >/dev/null 2>&1; then
            echo "  pyqtgraph + Qt : OK (roi_editor.py GUI will work)"
        else
            echo "  pyqtgraph + Qt : pyqtgraph imports but Qt failed to start."
            echo "                   Usually missing system libraries:"
            echo "                     Debian/Ubuntu : sudo apt install libgl1 libegl1 libxkbcommon-x11-0"
            echo "                     Fedora/RHEL   : sudo dnf install mesa-libGL libxkbcommon-x11"
            echo "                   (all headless scripts still work without it)"
        fi
    else
        echo "  pyqtgraph + Qt : MISSING -- roi_editor.py needs it. Install with:"
        echo "                     pip install pyqtgraph pyside6"
        echo "                   (all headless scripts still work without it, and so"
        echo "                    does 'from roi_editor import load_roi_set')"
    fi
}
...
# Every third-party import the headless scripts need. pyqtgraph is deliberately
# NOT here: it is GUI-only, so a missing one must not read as a broken install --
# report_gui() checks it separately, below.
...
# pyqtgraph/Qt is optional (GUI-only), so it is reported separately and never fatal.
if [[ "${MODE}" == "conda" ]]; then
    GUI_PY="$("${RUN_PY[@]}" -c 'import sys; print(sys.executable)')"
else
    GUI_PY="${VENV_PY}"
fi
report_gui "${GUI_PY}"
...
==============================================================
 ...
 Then, from ${REPO_ROOT}:
   ...
   python roi_editor.py --help        # needs pyqtgraph + Qt
